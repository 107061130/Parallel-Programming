...
void input(char* infile) {
    ...
    int* pair = (int*)malloc(3 * m * sizeof(int));
    fread(pair, sizeof(int), 3 * m, file);
    #pragma omp parallel for
    for (int i = 0; i < 3 * m; i += 3) Dist[pair[i]][pair[i + 1]] = pair[i + 2];
...
void output(char* outFileName) {
    ...
    #pragma omp parallel for
    for (int i = 0; i < n * n; ++i) temp[i] = Dist[i / n][i % n];

    fwrite(temp, sizeof(int), n*n, outfile);
    free(temp);
    //for (int i = 0; i < n; ++i) fwrite(Dist[i], sizeof(int), n, outfile);
    fclose(outfile);
}
