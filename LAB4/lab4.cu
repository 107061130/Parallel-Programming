...
#define BLOCK_SIZE_Y 96
#define COPY_SIZE_X (BLOCK_SIZE + 4)
#define COPY_SIZE_Y (BLOCK_SIZE_Y + 4)
#define DEV_NO 0
//cudaDeviceProp prop;
...
__global__ void sobel(unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    // copy s
    __shared__ unsigned char shared_s[COPY_SIZE_X * COPY_SIZE_Y * 3];
    const unsigned offset_y = blockIdx.y * BLOCK_SIZE_Y;
    const unsigned offset_x = blockIdx.x * blockDim.x;

    const int ID = threadIdx.y * BLOCK_SIZE + threadIdx.x;
    for (int i = ID; i < COPY_SIZE_X * COPY_SIZE_Y * channels; i += BLOCK_SIZE * BLOCK_SIZE) {
        int cha = i % channels;
        int col = (i / channels) % COPY_SIZE_X;
        int row = (i / channels) / COPY_SIZE_X;
        if (bound_check(offset_x + col - xBound, 0, width) && bound_check(offset_y + row - yBound, 0, height)) {
            shared_s[i] = s[channels * (width * (offset_y + row - yBound) + (offset_x + col - xBound)) + cha];
        }
    }
    __syncthreads();

    // location
    const unsigned x = offset_x + threadIdx.x;
    int val00, val01, val02, val10, val11, val12;
    if (x >= width) return;

    /* Z axis of mask */
    for (int k = 0; k < BLOCK_SIZE_Y / BLOCK_SIZE; k++) {
        unsigned y = offset_y + threadIdx.y + k * BLOCK_SIZE;
        if (y >= height) return;
        ...
        for (int v = -yBound; v <= yBound; ++v) {
            for (int u = -xBound; u <= xBound; ++u) {
                if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                    int yi = threadIdx.y + k * BLOCK_SIZE + v + yBound;
                    int xi = threadIdx.x + u + xBound;
                    const unsigned char R = shared_s[channels * (COPY_SIZE_X * yi + xi) + 2];
                    const unsigned char G = shared_s[channels * (COPY_SIZE_X * yi + xi) + 1];
                    const unsigned char B = shared_s[channels * (COPY_SIZE_X * yi + xi) + 0];
...
int main(int argc, char** argv) {
    ...
    const int block_x = (width + BLOCK_SIZE - 1) / BLOCK_SIZE;
    const int block_y = (height + BLOCK_SIZE_Y - 1) / BLOCK_SIZE_Y;
