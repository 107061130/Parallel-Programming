...
__global__ void sobel(unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    int  x, y = threadIdx.x + blockIdx.x * blockDim.x, i, v, u;
    int  R, G, B;
...
int main(int argc, char** argv) {
    ...
    int* mask_ = NULL;
    cudaMalloc((void**)&host_s_, width * height * channels * sizeof(unsigned char));
    cudaMalloc((void**)&host_t_, width * height * channels * sizeof(unsigned char));
    //cudaMalloc((void**)&mask_, MASK_N * MASK_Y * MASK_X * sizeof(int));

    /* Hint 2 */
    // cudaMemcpy(...) copy source image to device (filter matrix if necessary)
    cudaMemcpy(host_s_, host_s, width * height * channels * sizeof(unsigned char), cudaMemcpyHostToDevice);
    //cudaMemcpy(mask_, mask, MASK_N * MASK_Y * MASK_X * sizeof(int), cudaMemcpyHostToDevice);

    /* Hint 3 */
    // acclerate this function
    int threads_num = 64;
    int blocks_num = (width * height) / threads_num + 1;
    sobel << < blocks_num, threads_num >> > (host_s_, host_t_, height, width, channels);

    /* Hint 4 */
    // cudaMemcpy(...) copy result image to host
    cudaMemcpy(host_t, host_t_, width * height * channels * sizeof(unsigned char), cudaMemcpyDeviceToHost);
